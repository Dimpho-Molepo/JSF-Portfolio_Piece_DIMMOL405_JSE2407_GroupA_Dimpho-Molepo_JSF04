<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="absolute top-20 left-4 sm:left-6 lg:left-8">
        <button @click="goBack" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded transition duration-300 ease-in-out">
          Back to Products
        </button>
      </div>
      
      <div class="mt-28 sm:mt-32">
        <div v-if="productStore.error" class="text-red-500 text-xl font-bold text-center">{{ productStore.error }}</div>
        <div v-else-if="productStore.loading">
          <ProductDetailSkeleton />
        </div>
        <div v-else-if="product" class="mt-6 sm:mt-8 lg:flex lg:items-start lg:max-w-6xl xl:max-w-7xl mx-auto">
          ...
          <div class="w-full lg:w-3/5 lg:pl-8 space-y-4">
            ...
            <button class="bg-cyan-700 hover:bg-cyan-900 w-full sm:w-auto text-white font-bold py-2 px-4 rounded transition duration-300 ease-in-out">
              Add To Cart
            </button>
